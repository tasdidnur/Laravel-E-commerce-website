...
{{-- alert  --}}       
    ...
    <div class="col-lg-12 mb-4 order-0">
        <div class="card">
            <h5 class="card-header">All Caregoryes</h5>
            <div class="card-body">
                <div class="table-responsive text-nowrap">
                   <form method="POST" action="{{ url('/dashboard/category/mark_delete') }}" enctype="multipart/form-data">
                    @csrf 
                ...
                @foreach ($categoryes as $category) 
                    <tr>
                        ...
                        <td>{{ optional($category->created_at)->format('d-m-Y | h:i:s A') }}</td>
                        <td>{{ optional($category->editor_info)->name }}</td>
                        <td>{{ optional($category->updated_at)->format('d-m-Y | h:i:s A') }}</td>
                        <td>
                        <div class="dropdown">
                            ...
                            <a class="dropdown-item" href="{{ url('/dashboard/category/edit/'.$category->cat_slug) }}"><i class="bx bx-edit-alt me-1"></i> Edit</a>
                            <a class="dropdown-item" id="softDelete" data-id={{ $category->id }} href="#" data-bs-toggle="modal" data-bs-target="#modalToggle"><i class="bx bx-trash me-1"></i> Delete</a>
                            </div>
                        </div>
                        </td>
                    </tr>
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content bg-danger">
          ...
          <form method="POST" action="{{ url('/dashboard/category/delete') }}">
            @csrf      
          <div class="modal-body text-white">Are you sure delete this?</div>
